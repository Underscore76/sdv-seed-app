...
{
    public class CompressedResult : ICompressedRemixBundles
    {
        public CompressedResult(UInt128 state) : base(state) { }

        public override List<string> GetFields()
        {
            return CompressedFlags.GetFields(State);
        }
    }
    ...
    public class CompressedFlags
    {
        ...
        {
            UInt128 flag = new UInt128();
            FieldInfo[] Flags = typeof(CompressedFlags).GetFields();
            for (int i = 0; i < Flags.Length; i++)
            {
                Flags[i].SetValue(flag, (UInt128)0b1 << i);
        ...
        //Fish Tank
            ...
            FISH_SPECIALTY, FISH_QUALITY, FISH_MASTER,
        ...
        public static List<string> GetFields(UInt128 state)
        {
            List<string> results = [];
            FieldInfo[] Flags = typeof(CompressedFlags).GetFields();
            for (int i = 0; i < Flags.Length; i++)
            {
                if ((state & (UInt128)Flags[i].GetValue(null)) != 0)
                {
                    results.Add(Flags[i].Name);
                }
            }
            return results;
        }
